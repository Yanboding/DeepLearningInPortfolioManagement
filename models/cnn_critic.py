...
class Value(gpytorch.models.ExactGP, nn.Module):
    ...
    def __init__(self, state_dim, action_dim, fisher_num_inputs, gp_likelihood, feature_extractor=None):
    ...
    def forward(self, x, state_multiplier, fisher_multiplier, only_state_kernel=False):
        ...
        states = x[:, :self.state_element_num].reshape(-1, *self.state_dim)
        extracted_features = self.feature_extractor(states)
